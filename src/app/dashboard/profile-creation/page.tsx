...
export default function ProfileCreation() {
  ...
  const updateURL = (newRole?: string, newStep?: number) => {
    ...
    const newURL = queryString ? `/dashboard/profile-creation?${queryString}` : '/dashboard/profile-creation';
        ...
        <div className={`${sidebarCollapsed ? 'w-16' : 'w-64'} bg-white border-r border-gray-200 flex flex-col transition-all duration-200 ease-out will-change-[width]`}>
          ...
          <nav className="flex-1 p-4 space-y-2">
            {sidebarItems.map((item) => (
              <Link
                key={item.id}
                href={item.id === 'overview' ? '/dashboard/overview' : '/dashboard/profile-creation'}
