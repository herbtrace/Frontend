    ...
    <div className="h-screen bg-white flex flex-col">
      ...
      <main className="flex-1 p-2 overflow-hidden">
        <div className="h-full max-w-5xl mx-auto flex flex-col">
          {/* Message Bar - Compact */}
          {message && (
            <div className={`mb-2 p-2 rounded border text-sm ${message.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
          ...
          {!selectedRole ? (
            ...
            <div className="flex flex-col justify-center min-h-0">
              <div className="mb-4 text-center">
                <h1 className="text-xl font-semibold text-slate-900 mb-1">Create Profile</h1>
                <p className="text-sm text-slate-600">Select your role in the herbal supply chain</p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 justify-items-center">
                {roleOptions.map((role) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-8 h-8 ${role.color} rounded-md flex items-center justify-center mx-auto mb-2 group-hover:scale-105 transition-transform`}>
                        <IconComponent className="w-4 h-4 text-white" />
                      </div>
                      <h3 className="font-medium text-slate-900 text-xs mb-1">{role.label}</h3>
                      <p className="text-[10px] text-slate-500 line-clamp-1">{role.description}</p>
                    </button>
                  );
                })}
              </div>
                ...
                <div className="flex items-center space-x-2">
                  {(() => {
                    ...
                    if (selectedRoleOption) {
                      ...
                      return (
                        <>
                          <div className={`w-7 h-7 ${selectedRoleOption.color} rounded-md flex items-center justify-center`}>
